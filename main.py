...
def generate_innings_break_analysis(teams, first_innings_score, venue):
    prompt = f"""You are CricAI's live match analyst. The first innings has just ended. Write
a short innings-break analysis for the CricAI app with two parts.
...
{{"recap": "2 sentence recap of how the first innings went and the final total",
"chase_analysis": "2-3 sentence analysis of how tough the target is to chase, referencing the total, required run rate, and venue tendencies if relevant",
"updated_prediction": {{"predicted_winner": "Team Name", "confidence": 60, "reasoning": "1-2 sentence updated reasoning based on the target set"}}}}"""
    ...
    try:
        ...
        return {
            "recap": "First innings complete.",
            "chase_analysis": "Chase analysis unavailable.",
            "updated_prediction": {"predicted_winner": teams[0] if teams else "TBD", "confidence": 50, "reasoning": "Insufficient data"}
        }


@app.get("/innings-break/{match_id}")
def get_innings_break(match_id: str):
    cache_key = f"innings_break_{match_id}"
    cached = cache_get(cache_key)
    if cached:
        return cached
    ...
    first_innings_score = score[0] if score else {}

    analysis = generate_innings_break_analysis(teams, first_innings_score, venue)
